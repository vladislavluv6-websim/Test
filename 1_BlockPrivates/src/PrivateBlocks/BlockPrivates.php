...
final class BlockPrivates extends PluginBase implements Listener{
    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args) : bool{
        ...
        if(strtolower($args[0] ?? "help") === "help"){
            $this->sendHelpForm($sender);
            return true;
        }
    ...
    public function onBreak(BlockBreakEvent $event) : void{
        ...
        if($regionId !== null && (int) $this->regions[$regionId]["x"] === $pos->getFloorX() && (int) $this->regions[$regionId]["y"] === $pos->getFloorY() && (int) $this->regions[$regionId]["z"] === $pos->getFloorZ()){
            ...
            $event->getBlock()->getPosition()->getWorld()->addSound($pos->add(0.5, 0.5, 0.5), new XpLevelUpSound(2), [$event->getPlayer()]);
    ...
    private function makeRegion(array $type, Player $player, World $world, int $x, int $y, int $z) : array{
        ...
        return [
            ...
            "minX" => $x - $radius,
            "maxX" => $x + $radius,
            "minY" => max($world->getMinY(), $y - $radius),
            "maxY" => min($world->getMaxY(), $y + $radius),
            "minZ" => $z - $radius,
            "maxZ" => $z + $radius,
        ];
    }

    /** @return array{name:string,title:string,color:string,radius:int}|null */
    private function getPrivateType(Block $block) : ?array{
        $id = $block->getTypeId();
        return match($id){
            VanillaBlocks::IRON()->getTypeId() => ["name" => "Железный приват", "title" => TF::GRAY . "Железный приват", "color" => TF::GRAY, "radius" => 5],
            VanillaBlocks::DIAMOND()->getTypeId() => ["name" => "Алмазный приват", "title" => TF::AQUA . "Алмазный приват", "color" => TF::AQUA, "radius" => 8],
            VanillaBlocks::EMERALD_ORE()->getTypeId() => ["name" => "Изумрудный приват", "title" => TF::GREEN . "Изумрудный приват", "color" => TF::GREEN, "radius" => 15],
            VanillaBlocks::NETHERITE()->getTypeId() => ["name" => "Незеритовый приват", "title" => TF::LIGHT_PURPLE . "Незеритовый приват", "color" => TF::LIGHT_PURPLE, "radius" => 20],
            VanillaBlocks::ANCIENT_DEBRIS()->getTypeId() => ["name" => "Древние обломки", "title" => "", "color" => TF::LIGHT_PURPLE, "radius" => 30],
            default => null,
        };
    }
    ...
    private function intersectsAny(array $region) : bool{
        foreach($this->regions as $other){
            ...
            if($region["maxX"] >= $other["minX"] && $region["minX"] <= $other["maxX"] && $region["maxY"] >= $other["minY"] && $region["minY"] <= $other["maxY"] && $region["maxZ"] >= $other["minZ"] && $region["minZ"] <= $other["maxZ"]){
                return true;
            }
        }
        return false;
    ...
    private function getRegionIdAt(World $world, Vector3 $pos) : ?string{
        ...
        foreach($this->regions as $id => $region){
            if($world->getFolderName() === $region["world"] && $x >= $region["minX"] && $x <= $region["maxX"] && $y >= $region["minY"] && $y <= $region["maxY"] && $z >= $region["minZ"] && $z <= $region["maxZ"]){
                return (string) $id;
            }
        }
        return null;
    ...
    /** @param array<string, mixed> $region */
    private function removeHologram(array $region) : void{
        if((string) ($region["title"] ?? "") === ""){
            return;
        }
        $world = $this->getServer()->getWorldManager()->getWorldByName((string) $region["world"]);
        ...
        $particle = new FloatingTextParticle("", "", true);
        $pos = new Vector3((float) $region["x"] + 0.5, (float) $region["y"] + 1.35, (float) $region["z"] + 0.5);
    ...
    private function sendHelpForm(Player $player) : void{
        $player->sendForm(new class implements Form{
            /** @return array<string, mixed> */
            public function jsonSerialize() : array{
                return [
                    "type" => "form",
                    "title" => TF::GOLD . "✦ " . TF::YELLOW . "Приваты" . TF::GOLD . " ✦",
                    "content" => TF::WHITE . "Поставь блок привата без шифта, чтобы создать защищённую область вокруг блока.\n\n" .
                        TF::GRAY . "• Железный блок: " . TF::WHITE . "5 блоков в каждую сторону\n" .
                        TF::AQUA . "• Алмазный блок: " . TF::WHITE . "8 блоков в каждую сторону\n" .
                        TF::GREEN . "• Изумрудная руда: " . TF::WHITE . "15 блоков в каждую сторону\n" .
                        TF::LIGHT_PURPLE . "• Незеритовый блок: " . TF::WHITE . "20 блоков в каждую сторону\n" .
                        TF::DARK_PURPLE . "• Древние обломки: " . TF::WHITE . "30 блоков в каждую сторону\n\n" .
                        TF::YELLOW . "/rg add <ник>" . TF::WHITE . " — добавить игрока в приват, где ты стоишь.\n" .
                        TF::YELLOW . "/rg remove <ник>" . TF::WHITE . " — удалить игрока из текущего привата.",
                    "buttons" => [
                        ["text" => TF::RED . "Назад"],
                    ],
                ];
